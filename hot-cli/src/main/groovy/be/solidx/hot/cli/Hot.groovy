...
class Hot {
    ...
    Hot(projectsFolder = "", projectName = "") {
...
usage: hot <command> <options>
 commands:
	...
	jwt:	                Add/remove JWT based authenticarin to your web app.
	...
	help:			        Print this help.
        ...
        def filteredArgs = args.length > 1 ? args[1..- 1] : []
        ...
        switch (args[0]) {
            ...
            case "jwt":
                if (args[1] == 'add') {
                    ...
                    cli.url args: 1, 'url of the JWK set endpoint for JWT signature validation (used for asymmetric signature)', required: false
                    cli.s args: 1, 'secret used to validate signature (used for HMAC sign algorithm)', required: false

                    try {
                        def options = cli.parse(filteredArgs[1..-1])
                        Project project = new Project(projectName, projectsFolder)
                        if (!options.url && !options.s) {
                    ...
                    def cli = new CliBuilder(usage: 'hot jwt rm -n <name>', posix: false)
                    ...
                    println 'Invalid command... Please use add or rm command'
                }
                break
    ...
    static class Project {
        ...
        Project(projectName = "", projectsFolderPath = "") {
            this.projectName = projectName
