...
class TestJWTAuthentication {
    ...
    def createServer = {
        ...
        ServletHolder clientAuthHolder = new ServletHolder(JWTServlet.class)
        clientAuthHolder.name = "jwt"
        servletContextHandler.addServlet(clientAuthHolder, "/jwt/*")
    ...
    @Before
    void before() {
        System.setProperty("configFileLocation", "be/solidx/hot/test/shows/jwt-secured-config.json")
        createServer()
        server.start()
    }
    ...
    void test() {
        ...
        client.buildRequest(options2).then { data, textStatus, response ->
            options.headers = [
                'Authorization':"Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyLCJpc3MiOiJvYXV0aDAiLCJhdWQiOiJzY2VsbCIsImV4cCI6NDEzMzI4NjAwMDAwMH0.tA5NXxpSG6XcjSyxQ0f-DdwzzeZHHKvPRy7U_-pYLQM"
            ]
