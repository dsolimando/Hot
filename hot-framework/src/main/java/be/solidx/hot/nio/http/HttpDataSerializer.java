    ...
    public byte[] serialize(Object data, MediaType mediaType) throws HttpDataSerializationException {
        ...
        try {
            if (data instanceof Map) {
                Map convertedMap = (Map) data;
                if (data instanceof NativeObject) {
                    convertedMap = jsMapConverter.toMap((NativeObject) data);
                } else if (data instanceof PyDictionary) {
                    convertedMap = pyDictionaryConverter.toMap((PyDictionary) data);
                }
                return serializeMap(convertedMap, mediaType);
            } else {
