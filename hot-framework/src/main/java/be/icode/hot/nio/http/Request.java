...
public abstract class Request<CLOSURE,MAP> implements Promise<CLOSURE> {
	...
	@Override
	public Promise<CLOSURE> _done(be.icode.hot.promises.Promise.DCallback callback) {
		return deferred._done(callback);
	}
	
	@Override
	public Promise<CLOSURE> _fail(be.icode.hot.promises.Promise.FCallback callback) {
	...
	private ChannelPipelineFactory buildChannelPipelineFactory() {
		return new ChannelPipelineFactory() {
			...
			public ChannelPipeline getPipeline() throws Exception {
				...
				if (ssl) {
					SSLContext sslContext = sslContextBuilder.buildSSLContext(options);
