...
import be.icode.hot.data.mongo.groovy.GroovyAsyncBasicDBProxy;
...
public class GroovyShow extends AbstractShow<Closure<?>,Map<String, Object>,CompiledScript> {
	...
	protected Map<String, AsyncDB<Closure<?>, Map<String, Object>>> buildAsyncDBMap(Map<String, DB<Map<String, Object>>> dbs) {
		...
		for (Entry<String, DB<Map<String, Object>>> entry: dbs.entrySet()) {
			if (entry.getValue() instanceof be.icode.hot.data.jdbc.groovy.DB) {
				adbs.put(entry.getKey(), new be.icode.hot.data.jdbc.groovy.GroovyAsyncDBProxy ((be.icode.hot.data.jdbc.groovy.DB) entry.getValue(), blockingThreadPool, eventLoop));
			} else {
