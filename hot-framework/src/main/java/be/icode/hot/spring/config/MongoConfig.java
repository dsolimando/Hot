...
import be.icode.hot.data.mongo.DBObjectMapTransformer;
import be.icode.hot.data.mongo.DBObjectNativeObjectTransformer;
import be.icode.hot.data.mongo.DBObjectPyDictionaryTransformer;
...
public class MongoConfig {
	...
	public Map<String,DB<Map<String, Object>>> groovyMongoDb () {
		...
		for (Entry<DataSource, Mongo> dataSource: mongoDBs().entrySet()) {
			dbmap.put(dataSource.getKey().getName(), new be.icode.hot.data.mongo.groovy.DB(
	...
	public Map<String, DB<NativeObject>> jsMongoDB () {
		...
		for (Entry<DataSource, Mongo> dataSource: mongoDBs().entrySet()) {
			dbmap.put(dataSource.getKey().getName(), new be.icode.hot.data.mongo.js.DB(					
					dataSource.getKey().getUsername(), 
					dataSource.getKey().getPassword(), 
					dataSource.getKey().getDatabase(), 
					dataSource.getValue(), 
